...
# Some functions in here are yet to be decided if they will removed/edited due to out of the scope/not compatible with the intentions of CorpLite.
class Utils(commands.Cog):
	...
	def get_avatar(self,user,server=True):
		# Check for the old syntax
		if hasattr(user,"avatar_url"):
			return next((x for x in (user.avatar_url,user.default_avatar_url) if x),None)
		# Check for new - and leverage the display_avatar first
		if server:
			return user.display_avatar.url
		return next((x.url for x in (user.avatar,user.display_avatar) if x),None)

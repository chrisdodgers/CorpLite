import discord, re, urllib.parse
from discord import app_commands
from discord.ext import commands
from Cogs import Message, DL, PickList
...
class IntelArk(commands.Cog):
    ...
    # IntelArk Slash Command:
    @app_commands.command(name="iark", description="Get Intel CPU/iGPU info")
    @app_commands.describe(cpu_model="Enter an Intel CPU model name (e.g. i7-10700K)")
    @app_commands.allowed_contexts(guilds=True, dms=True, private_channels=True)
    @app_commands.user_install()
    async def iark(self, interaction: discord.Interaction, cpu_model: str):
        ...
        message = None  # Above (defer) is what generates the interaction, but I'm unsure exactly why if I set this to edit the interaction it essentially makes 2 interactions (1 for "Gathering info..."
        # and the other for what is below. Leaving this commented out for now - might remove it.

        # Query the ARK, and try to return a response
        try:
            # Query the ARK, and try to return a response
            response = await self.quick_search(cpu_model)
        except:
            response = []

            # Check if we got nothing
        if not len(response):
            args["description"] = f"No results returned for `{cpu_model}`."
            return await Message.Embed(**args).edit(interaction, message)
        ...
        if len(response) > 1:
            ...
            index, message = await PickList.Picker(
                message=message,
                title=f"Multiple Matches Returned For `{cpu_model}`:",
        ...
        if not response.get("list"):
            try:  # Maybe we got a product line - or something other than a specific CPU?
                ...
                args[
                    "description"] = "Could not retrieve specific data for this search.\nPlease follow the title link (or click [here]({})) to open your search results in your browser.".format(
                    response["Link"])
